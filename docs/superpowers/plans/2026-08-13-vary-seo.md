...
- Produces: a seven-section CORS-first `Vary` guide with four bounded HTTP examples, five related-header links, and a focused structural regression contract for cache matching, two-origin failure behavior, and corrected variant selection. Independent protocol review covers explanatory cache, CORS, preflight, wildcard, and cache-policy semantics.
