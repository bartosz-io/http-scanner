...
- Every structural production-content change must follow a demonstrated RED → GREEN test cycle; explanatory-copy corrections receive independent protocol re-review rather than exact-sentence tests.
...
**Interfaces:**
- Consumes: `getOpeningFrontmatter(source: string): string | undefined`, `parseFrontmatterList(frontmatter: string, field: string): string[]`, `collectMarkdownContractNodes(source: string)`, `validateHeaderGuideSource(slug: string, source: string): string[]`, and the existing shared Markdown renderer.
- Produces: a seven-section CORS-first `Vary` guide with four bounded HTTP examples, five related-header links, and a focused MDAST-backed structural regression contract for the complete H2 sequence, exact HTTP exchanges, rendered link destinations, cache matching, two-origin failure behavior, and corrected variant selection. Independent protocol review covers explanatory cache, CORS, preflight, wildcard, and cache-policy semantics.
...
Extend `collectMarkdownContractNodes` so the same MDAST traversal returns `httpCodeBlocks` from `code` nodes whose language is `http` and `linkDestinations` from `link` node URLs. The assertions above must consume those parsed values; raw fenced-block regexes and raw Markdown-link substring checks are not part of the contract.
...
- [x] **Step 3: Replace the related-header and reference lists**
...
- [x] **Step 4: Replace the short guide body with the approved content**
...
Do not add assertions for explanatory sentences or exact prose fragments. Those statements are reviewed independently against RFC 9110, RFC 9111, and Fetch in Task 2.

- [x] **Step 6: Run the infrastructure/caching category contract**
...
- [x] **Step 7: Inspect the scoped implementation diff**
...
These prose checks are review criteria, not substring assertions in Vitest.
...
- [x] **Step 1: Request independent spec-compliance review**
...
Any structural production-content correction must begin with a failing regression assertion or demonstrated failure of an existing assertion, followed by focused GREEN and the affected verification commands. An explanatory-copy correction must be justified against a normative source and receive scoped independent protocol re-review; do not add an exact-sentence assertion. Do not make prose changes solely because a reviewer prefers different style.

- [x] **Step 2: Request independent protocol and content-quality review**
...
- [x] **Step 4: Verify generated HTML structure and links**
...
- [x] **Step 5: Start the production-equivalent preview**
...
- [x] **Step 6: Perform desktop visual QA**
...
- [x] **Step 7: Perform mobile visual QA**
...
- [x] **Step 8: Capture the exact final implementation commit**
...
- [x] **Step 9: Update the Wave 1 roadmap row**
...
- [x] **Step 10: Add the completed-task record**
...
- [x] **Step 11: Advance the next task to Content-Security-Policy**
...
- [x] **Step 12: Mark completed plan actions factually**
...
- [x] **Step 13: Verify the documentation diff**
...
- [x] **Step 14: Commit roadmap completion**
...
- [x] **Step 15: Confirm the final handoff state**
